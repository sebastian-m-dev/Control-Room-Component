 ...
 * Reenvía GET /api/devices de Traccar usando la sesión guardada.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    upstream = await fetch(`${TRACCAR_BASE_URL}/api/devices`, {
      headers: {
    ...
    return NextResponse.json(
      {
        error: 'NETWORK_ERROR',
        message: 'No se pudo conectar con el servidor de Traccar para obtener los dispositivos.',
      },
      { status: 502 },
    );
  }
